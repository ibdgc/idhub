...
class TestFragmentValidatorIntegration:
    ...
    def test_process_blood_file_success(
        ...
        assert "gsid_resolution" in report
        assert "s3_location" in report
    ...
    def test_validation_failure_null_in_required(
        ...
        # Create data with a null value in a required column
        bad_data = pd.DataFrame(
            {
                "consortium_id": ["IBDGC001"],
                "sample_type": ["Blood"],
                "sample_id": [None],  # Explicitly add None
            }
        )
    ...
    def test_staging_outputs_created(
        ...
        assert validator.s3_client.upload_dataframe.call_count == 2
