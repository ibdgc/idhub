    ...
    def _get_or_create_subject(self, record: Dict[str, Any], center_id: int) -> str:
        ...
        with db_manager.get_connection() as conn:
            with db_manager.get_cursor(conn) as cursor:
                logger.debug(f"Looking for existing subject: center_id={center_id}, record_id={record_id}")
                ...
                logger.debug(f"Generated GSID: {gsid}, updating reserved record")
                ...
                logger.debug(f"Updated subject {gsid}, creating local_subject_id mapping")
